...
\setcheatsheetfontsize{6}  % Tune font size for readability/compactness
\setcheatsheetmargin{0.15in}
...
The following may be useful: $E_X(N(y))=\sum_{n=1}^\infty \bP^n(x,y)$
...
\begin{itemize}
\item $W(t)$ has covariance $\min(s,t)$, mean zero
...
Expoential: $f_Y(x)=\lambda\exp(-\lambda x), M_Y(t)=(1-t/\lambda)^{-1}$, $\mu=\lambda^{-1}, \sigma^2=\lambda^{-2}$.  $P(X<Y)=\frac{\lambda_x}{\lambda_x+\lambda_y}$
}
\nsubsection{Miscelaneous}{
Embeeded chain/Jump probabilities can also be computed as $Q(i,j)=q(i,j)/q_i$}
% End the cheat sheet
\endcheatsheet
\end{document}
