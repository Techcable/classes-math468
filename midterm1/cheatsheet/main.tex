...
To be a probability matrix, columns must sum to one and entries need to be nonnegative. \\
\begin{itemize}
...
\textbf{Thm 2.4}: (a) An equilibrium $\pi$ vanishes on all transient and null recurrent states. (b) A positive recurrent communication class supports a unique equilibrium $\pi(y)=\frac{1}{m_y}$ for $m_y$ finite.
}
% End the cheat sheet
\endcheatsheet
\end{document}
