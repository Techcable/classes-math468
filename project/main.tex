...
However, by definition of $g$ and $\alpha$, $\alpha$ is the largest $\lambda$ satisfying $A\bu\ge \lambda\bu$ where $\bu$ is an arbitrary vector.
So finding a larger value $\alpha+\epsilon$ satisfying the same inequality is a contadiction.
\vspace{2mm} \\
Lemma (d): I now claim that the vector $\bv_\alpha\ge \bvz$ satisfying $g(\bv_\alpha)=\alpha$ is unique.
Assume that this is not the case, so that there is a $\bv_\alpha'\ne \bv_\alpha$ s.t. $g(\bv_\alpha')=g(\bv_\alpha)=\alpha$.
As argued in part (c), we can assume these vectors $\bv_\alpha,\bv_\alpha'$ are unit vectors.
% The \bv_\alpha should not be chosen until part (d).
% In particular, the unit vector argument is only to argue for existence.
% It does not constrain the vectors we argue about in (c)
%
% This is so that we can later impose a different uniqueness constraint,
% that the sum of the coefficients  is one.
\end{document}